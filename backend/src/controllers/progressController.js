...
exports.submitQuiz = async (req, res, next) => {
  try {
    ...
    await updateLearnerStreak(req.user._id);
    await updateLearnerMetrics(req.user._id);
    ...
    // If the learner scores >= 80%, generate a new, harder set of 5 questions
    if (score >= 80) {
      try {
        ...
        // We gracefully silently fail so the learner still submits their current progress.
      }
    }
