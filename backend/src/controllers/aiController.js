...
exports.getStudyPlan = async (req, res, next) => {
  try {
    const { goal, hoursPerWeek, weeks, courseId, syllabusContext } = req.body;
    const user = req.user;
...
exports.generateAIQuiz = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'learner') {
      const recent = await Progress.find({ learner: req.user._id, course: courseId })
...
exports.getFeedback = async (req, res, next) => {
  try {
    const user = req.user;
    const weakTopics = await getWeakTopics(user._id);
    const recent = await Progress.find({ learner: user._id })
