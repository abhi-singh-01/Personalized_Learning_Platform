...
export default function StudyPlan() {
  ...
  const [syllabusContext, setSyllabusContext] = useState(null);
  ...
  useEffect(() => {
    ...
    // Load syllabus context if coming from AIVideoPanel
    const syllabusFrom = searchParams.get('syllabusFrom');
    if (syllabusFrom) {
      api.get('/ai/transcript/' + syllabusFrom).then((res) => {
        if (res.data?.syllabus?.topics?.length) {
          setSyllabusContext(res.data.syllabus);
          setForm(f => ({
            ...f,
            goal: `Master all topics from the video lecture: ${res.data.syllabus.topics.map(t => t.title).join(', ')}`,
          }));
        }
      }).catch(() => {});
    }
    ...
    <div className="space-y-6">
      ...
      {syllabusContext && (
        <div className="card bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
          <h3 className="font-semibold text-sm text-purple-700 dark:text-purple-400 mb-2">📚 Syllabus Context Loaded</h3>
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">This study plan will be generated based on topics extracted from your video lecture:</p>
          <div className="flex flex-wrap gap-1.5">
            {syllabusContext.topics.map((t, i) => (
              <span key={i} className="text-xs px-2 py-0.5 bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 rounded">{t.title}</span>
            ))}
          </div>
        </div>
      )}
